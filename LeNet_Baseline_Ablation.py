...
# Full Dataset values -- Use this instead of the above to train them model on the full dataset.
...
"""
ImageDataGenerator is used to perform image augmentation, and removing it so that the model
uses the original un-augmented images is the ablation in my ablation experiment.
"""
...
The change made from the normal baseline model is that, when fitting the model to create the history,
the train_images and train_labels are no longer run through Image Data Generator (data_gen).
"""
...
history = lenet_model.fit(train_images, train_labels, validation_data=(val_images, val_labels), epochs=25)

# Save the model's predictions made during validation.
